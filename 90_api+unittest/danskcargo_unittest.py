...
class TestSoftDelete(unittest.TestCase):
    def test_aircraft_max_cargo_soft_delete(self):
        ...
        with Session(engine) as session:
            ...
            session.add_all(new_items)
            session.commit()
        dcsql.get_record(dcd.Aircraft, 1)  # henter en instans af et objekt af typen Aircraft med record_id = 1
        # act
        dcsql.delete_soft_aircraft(dcd.Aircraft)  # aircraft soft deletes
        # assert
        self.assertEqual(dcd.Aircraft.max_cargo_weight, -1)  # checker om max_cargo_weight er sat til -1 og derfor er soft_deleted
