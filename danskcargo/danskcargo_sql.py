...
def create_test_data():  # Optional. Used to test database functions before gui is ready.
    with Session(engine) as session:
        new_items = []
        new_items.append(Aircraft(weight=1200, destination="Oslo"))
...
def update_container(container):  # https://docs.sqlalchemy.org/en/14/tutorial/orm_data_manipulation.html#orm-enabled-update-statements
    ...
    with Session(engine) as session:
        session.execute(update(Aircraft).where(Aircraft.id == container.id).values(weight=container.weight, destination=container.destination))
        session.commit()  # makes changes permanent in database
...
def delete_soft_container(container):
    ...
    with Session(engine) as session:
        session.execute(update(Aircraft).where(Aircraft.id == container.id).values(weight=-1, destination=container.destination))
        session.commit()  # makes changes permanent in database
# endregion container
